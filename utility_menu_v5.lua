...
hook.Add("PostDrawOpaqueRenderables", "DrawEntityBoxes", function()
	...
	for _, ent in ipairs(ents.GetAll()) do
		if settings.propbox and (ent:GetClass():find("prop_physics") or ent:GetClass():find("prop_dynamic")) then
...
hook.Add("HUDPaint", "DrawNametags", function()
	...
	for _, ent in ipairs(ents.GetAll()) do
		...
		elseif settings.playernametag and ent:IsPlayer() and ent ~= ply and ent:Alive() then
			local pos = ent:EyePos():ToScreen()
...
hook.Add("CreateMove", "AutoBhop", function(cmd)
	local ply = LocalPlayer()
	if settings.autobhop and cmd:KeyDown(IN_JUMP) and not ply:IsOnGround() or ply:WaterLevel() > 1 or ply:GetMoveType() == MOVETYPE_NOCLIP then
