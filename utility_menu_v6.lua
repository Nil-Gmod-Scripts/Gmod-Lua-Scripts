			...
			if IsValid(ent) then
				...
				elseif ent:Alive() then
					if settings.npcbox and ent:IsNPC() then
						render.DrawWireframeBox(pos, Angle(0, 0, 0), obbMins, obbMaxs, Color(255, 155, 0), false)
			...
			if IsValid(ent) then
				...
				if ent:Alive() then
					if settings.npcinfo and ent:IsNPC() then
						draw.SimpleText(ent:GetClass(), "BudgetLabel", pos.x, pos.y + -12, Color(255, 155, 0), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM)
						draw.SimpleText("HP:" .. health, "BudgetLabel", pos.x, pos.y, Color(255, 155, 0), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM)
